...
#ifndef TOUCHSENSING_H_
#define TOUCHSENSING_H_
...
enum return_type_t              {RETURN_NOK = 0, RETURN_OK = 1};

class mooltipass_touch_sensing
{
 public:
  mooltipass_touch_sensing(void);
  uint8_t begin(void);
  ...
  uint8_t touchDetectionRoutine(uint8_t led_mask);
  uint8_t writeDataToTS(uint8_t reg, uint8_t data);
  uint8_t readDataFromTS(uint8_t reg, uint8_t* data);
