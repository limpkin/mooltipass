...
browserAction.show = function(callback, tab) {
	...
	chrome.browserAction.setIcon({
		tabId: tab.id,
		path: "/icons/" + browserAction.generateIconName(data.iconType, data.icon)
	});
...
browserAction.update = function(interval) {
	...
	if(data.intervalIcon) {
		...
		chrome.browserAction.setIcon({
			...
			path: "/icons/" + browserAction.generateIconName(null, data.intervalIcon.icons[data.intervalIcon.index])
		});
	}
}
...
browserAction.showDefault = function(callback, tab) {
	...
	if(!mooltipass.isUnlocked() || page.tabs[tab.id].errorMessage) {
		stackData.iconType = "gray";
	}

    if(page.tabs[tab.id].loginList.length > 0) {
        stackData.iconType = "red";
...
browserAction.generateIconName = function(iconType, icon) {
	...
	var name = "mooltipass-19";
	//name += (keepass.keePassHttpUpdateAvailable()) ? "new_" : "";
	name += (!iconType || iconType == "normal") ? "" : '-' + iconType;
	name += ".png";
