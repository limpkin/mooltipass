...
mooltipass.latestExtensionVersionUrl = 'https://raw.githubusercontent.com/limpkin/mooltipass/master/authentication_clients/chromeipass.ext/manifest.json';
...
mooltipass.latestExtension = (typeof(localStorage.latestExtension) == 'undefined') ?
                                {"version": 0, "versionParsed": 0, "lastChecked": null} :
                                JSON.parse(localStorage.latestExtension);
...
mooltipass.isUnlocked = function() {
...
mooltipass.getLatestExtensionVersion = function()
{
	var xhr = new XMLHttpRequest();
	xhr.open("GET", mooltipass.latestExtensionVersionUrl, false);
	...
	try {
        ...
        mooltipass.latestExtension.version = version;
        mooltipass.latestExtension.versionParsed = parseInt(version.replace(/\./g,''));
	} catch (e) {
		console.log("Error: " + e);
	}

	if (version != -1) {
		localStorage.latestExtension = JSON.stringify(mooltipass.latestExtension);
	}
	mooltipass.latestExtension.lastChecked = new Date();
}

mooltipass.getLatestAppVersion = function()
{
	var xhr = new XMLHttpRequest();
	...
	try {
		...
		manifest = JSON.parse(xhr.responseText);
        version = manifest.version;
        mooltipass.latestApp.version = version;
        mooltipass.latestApp.versionParsed = parseInt(version.replace(/\./g,''));
	} catch (e) {
		console.log("Error: " + e);
	}

	if (version != -1) {
		localStorage.latestApp = JSON.stringify(mooltipass.latestApp);
	}
	mooltipass.latestApp.lastChecked = new Date();
}
